...
# Oyun bitiş koşulu (örneğin, belirli bir sayıya ulaşıldığında)
while True:
    sc.update()
    ...
    # Oyuncu karakteri ve topun çarpışmasını kontrol et
    if (left_pad.xcor() + 30 > hit_ball.xcor() > left_pad.xcor() - 30 and
        left_pad.ycor() + 50 > hit_ball.ycor() > left_pad.ycor() - 50):
        hit_ball.setx(left_pad.xcor() + 30)
        hit_ball.dx *= -1

    if (right_pad.xcor() - 30 < hit_ball.xcor() < right_pad.xcor() + 30 and
        right_pad.ycor() + 50 > hit_ball.ycor() > right_pad.ycor() - 50):
        hit_ball.setx(right_pad.xcor() - 30)
        hit_ball.dx *= -1

    # Kazananı kontrol et ve ekrana yaz
    if left_player == 5:
        sketch.clear()
